process GLIMPSE_PHASE {
    ...
    input:
        tuple val(meta) , path(input), path(input_index), path(samples_file), val(input_region), val(output_region), path(reference), path(reference_index), path(map), val(chr)

    output:
        tuple val(meta), path("*.{vcf,bcf,vcf.gz,bcf.gz}"), val(chr), emit: phased_variants
    ...
    def prefix = task.ext.prefix ?: "${meta.id}.${chr}_${input_region.replace(":","_")}"
