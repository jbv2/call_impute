process BCFTOOLS_REHEADER {
    ...
    input:
    tuple val(meta), path(vcf), path(header), path(samples), val(chr)
    tuple val(meta2), path(fai)

    output:
    tuple val(meta), path("*renamed.{vcf,vcf.gz,bcf,bcf.gz}"), path("*.{csi,tbi}"), val(chr), emit: vcf 
    ...
    bcftools \\
        ...
        | bcftools view \\
        $args2 \\
        --output ${prefix}.renamed.${extension}
